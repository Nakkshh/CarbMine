...
const RegisterPage = () => {
    const [registerData, setRegisterData] = useState({ CoalName: '', email: '', password: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        if (!emailPattern.test(registerData.email)) {
            setMessage('Invalid email format.'); return;
        }
        ...
        setMessage('');
        try {
            await firebase.addUser(registerData.CoalName, registerData.email, registerData.password);
            navigate('/');
        } catch (error) {
            switch (error.code) {
                case 'auth/email-already-in-use': setMessage('This email is already registered.'); break;
                case 'auth/invalid-email': setMessage('Invalid email address.'); break;
                case 'auth/weak-password': setMessage('Password is too weak.'); break;
                default: setMessage('Failed to register. Please try again.');
            }
        } finally {
            setLoading(false);
        }
            ...
            <div className="hidden lg:flex flex-1 flex-col justify-between p-12 relative overflow-hidden"
                ...
                <div className="relative z-10 flex flex-col gap-6">
                    ...
                    <h2 className="text-4xl font-black text-white leading-tight">
                        Join India's<br />
                        <span style={{ color: 'var(--primary)' }}>Green Future.</span>
                    </h2>
                    <p className="text-sm text-white/50 max-w-xs leading-relaxed">
                        Register your coal mine and start tracking, neutralizing, and profiting from your carbon actions today.
                    </p>

                    {/* Feature checklist */}
                    <div className="flex flex-col gap-3 mt-2">
                        {[
                            'Free carbon footprint tracking',
                            ...
                            <div key={i} className="flex items-center gap-3">
                                <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                                    style={{ background: 'rgba(0,240,32,0.2)' }}>
                ...
                <div className="w-full max-w-sm flex flex-col gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-sm mt-2" style={{ color: 'var(--text-secondary)' }}>
                            Register your coal mine on CarbMine
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <input
                                type="text" name="CoalName"
                                placeholder="e.g. Jharia Coal Mine"
                                    ...
                                    <div className="flex gap-1 flex-1">
                                        {[1, 2, 3].map(i => (
                                            <div key={i} className="h-1 flex-1 rounded-full transition-all duration-300"
                                                style={{
                                                    background: registerData.password.length >= i * 3
                                                        ? i === 1 ? '#ef4444' : i === 2 ? '#f59e0b' : 'var(--primary)'
                                                        : 'var(--border)'
                                                }} />
                                        ))}
                                    </div>
                                    <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                                        {registerData.password.length < 3 ? 'Weak' : registerData.password.length < 6 ? 'Fair' : 'Strong'}
                                    </span>
                                </div>
                            )}
                        </div>
